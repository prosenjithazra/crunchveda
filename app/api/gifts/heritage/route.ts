...
const mergeLocalHeritageFallback = (backendData: any) => {
  ...
  if (!merged.categoryTitle) {
    merged.categoryTitle = getVal("heritageHeading") || "The Heritage";
  }
  ...
  if (!merged.products || merged.products.length === 0) {
    ...
    const heritageCat = cats.find(c => c.categoryTitle?.toLowerCase()?.includes("heritage"));
    if (heritageCat && heritageCat.products) {
      merged.products = heritageCat.products;
    } else {
      ...
      merged.products = parseItems(getVal("heritageItems"), "6a4bac669970b0ba46cf9ce");
    }
  }
...
export async function GET() {
  ...
  const localRes = await tryFetch(`${LOCAL_BACKEND}/gifts/heritage`);
  if (isSuccessResponse(localRes)) {
    const category = localRes.data?.category || {};
    const merged = mergeLocalHeritageFallback(category);
  ...
  const remoteRes = await tryFetch(`${REMOTE_BACKEND}/gifts/heritage`);
  ...
  const category = mergeLocalHeritageFallback({});
  return NextResponse.json({
    success: true,
    status: "success",
    data: { category }
...
export async function PUT(request: Request) {
  ...
  try {
    const localRes = await fetch(`${LOCAL_BACKEND}/gifts/heritage`, { ...options, cache: "no-store" });
    if (localRes.ok) {
  ...
  if (!savedToBackend) {
    try {
      const remoteRes = await fetch(`${REMOTE_BACKEND}/gifts/heritage`, { ...options, cache: "no-store" });
      if (remoteRes.ok) {
  ...
  const heritageIdx = localCategories.findIndex(c => c.categoryTitle?.toLowerCase()?.includes("heritage"));
  if (heritageIdx !== -1) {
    localCategories[heritageIdx] = category;
  } else {
    localCategories[0] = category;
  }
  ...
  const currentSeasonalCat = localCategories.find(c => c.categoryTitle?.toLowerCase()?.includes("seasonal")) || { categoryTitle: "The Seasonal", products: [] };

  record.fields = [
    { id: "heritageHeading", label: "Left column heading", type: "text", value: category.categoryTitle || "" },
    { id: "heritageItems", label: "Heritage items", type: "textarea", value: serializeItems(category.products || []) },
    { id: "seasonalHeading", label: "Right column heading", type: "text", value: currentSeasonalCat.categoryTitle || "" },
    { id: "seasonalItems", label: "Seasonal items", type: "textarea", value: serializeItems(currentSeasonalCat.products || []) },
  ...
  return NextResponse.json({
    success: true,
    status: "success",
    message: "Heritage category updated successfully.",
    data: { category },
    savedToBackend
  });
}
